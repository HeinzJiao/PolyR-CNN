...
class MaskRasterizationLoss(nn.Module):
    ...
    def forward(self, preds, targets, lid=0):
        """
        :params preds: torch.Size([num_gt_boxes_batch, num_queries_per_poly*2])
        :params targets: torch.Size([num_gt_boxes_batch, num_queries_per_poly*2])
        :params target_len: torch.Size([num_gt_boxes_batch])
        """

        resolution = self.rasterize_at[lid]  # [64, 64]

        target_masks = []
        pred_masks = []
        for i in range(targets.shape[0]):  # num_gt_boxes_batch

            tgt_poly_single = targets[i].view(-1, 2).unsqueeze(0)  # torch.Size([1, num_queries_per_poly*2, 2])
            pred_poly_single = preds[i].view(-1, 2).unsqueeze(0)  # torch.Size([1, num_queries_per_poly*2, 2])
